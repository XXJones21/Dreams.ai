...
- **Cenedril** (`core/agents.py`) authors the Flux shot-composition prompt (**first-person POV enforced** — camera = the dreamer's own eyes; figures/bodies/silhouettes/from-behind are forbidden in the prompt) → writes `pre_production.cenedril_shot_composition`. A **second LLM call** produces `cenedril_camera_motion`, a *gentle* observe-only motion directive (slow drift/sway only; lurch/shake/spin/warp/morph/glitch/blur are forbidden — these were the main image↔video drift driver). The image prompt is cleaned of LLM preamble ("Here is…") and stray quotes before use.
- **CenedrilImageGenerator** reads the shot prompt, calls `engram_comfy.ComfyImageGenerator(...).generate_image(...)`, writes result to `post_production.image_generation`.
- **CenedrilVideoGenerator** runs only when `state["generate_video"]` is set; default workflow is **`scene_video_ltx2`** (LTX-2.3 GGUF two-stage **image-to-video**, portrait 9:16, 241 frames ≈ 10 s, ~7.6 min on the 4080); `state["video_cinematic"]` selects Wan2.2. **It is now true I2V**: the generated Flux frame is the video's start frame (`image_input = image_block["filepath"]` — a local path, since the workflow loads via `VHS_LoadImagePath`; NOT the `asset_url`). The **video prompt drives MOTION only** = POV preamble + `cenedril_camera_motion` (it does *not* re-describe the scene — the start frame already sets it). I2V conditioning strength 0.9. Set workflow override `i2v_bypass=true` for pure T2V. **Working LTX-2.3 GGUF recipe & gotchas: see Engram/memory and the `scene_video_ltx2.json` `_meta.notes` — the critical one is `DualCLIPLoaderGGUF` clip2 = `ltx-2.3-22b-dev_embeddings_connectors.safetensors` (NOT text_projection), and the pack needs the `pad = F.pad` shim in `ComfyUI-LTXVideo/pyramid_blending.py` to load at all.**
...
**E2E milestones & progress (2026-05-25):**
- **E2E #1 — linear pipeline (prompt → single video out): ✅ working on phone.** Async dream creation (background thread + single-flight 409 guard, dodges the Cloudflare 524 tunnel timeout) → progressive display (poll `.imn`, seed media from it, WS heartbeat) → portrait video plays. Asset URLs are rewritten `127.0.0.1:8188` → same-origin via the Vite `/comfy` proxy so the phone can load them.
- **E2E #2 — interaction (tap-to-continue): in progress (the major milestone).** *Step 1 (T2V→I2V start frame): done* (see `CenedrilVideoGenerator` above). *Step 2 frontend: done* — `InteractivePlayerPage` does fullscreen-on-play, tap-to-pause, canvas frame-capture (normalized tap x,y), and shows the scene's `.imn` `actions` as tappable chips, POSTing to a **stubbed** `/continue`. *Step 2 backend (pending, `#30`):* `POST /api/dream/{id}/continue` (multipart paused-frame PNG + action + tap x/y → run the I2V stage with the paused frame as `image_input` and the action as the motion prompt → append a scene) + expose `scene_context`/`actions` in the `imn_to_dreamcard` GET projection. *SAM 3 segmentation + mask conditioning come after this precursor.*
...
- **Image** workflow JSONs live in the `engram_comfy` package: `scene_image_sdxl_turbo.json`, `scene_image_sdxl_lora.json`. The **video** workflow `scene_video_ltx2.json` (the default) lives in **ComfyLocalMCP** at `D:\Tools\ComfyLocalMCP\comfy_local_mcp\workflows\`, driven via `ComfyVideoGenerator`. Duration is the `frames` override (node 10, must be `8n+1`: 121≈5 s, 241≈10 s); I2V knobs are `image_input`, `i2v_strength`, `i2v_bypass`.
- **Worktrees (parallel dev):** `D:\Dreams.ai` = `main`; `D:\Dreams.ai-backend` = `feat/e2e-linear-pipeline` (I2V graft + agent anti-drift fixes); `D:\Dreams.ai-frontend` = `feat/mobile-redesign` (mobile UI + tap-to-pause player). Feature branches are **not yet merged to `main`**.
...
- API server CORS hardcoded to a Netlify URL; needs env-driven config for local dev. (Frontend now reaches the backend via env-driven `API_BASE_URL` + Vite `/api` & `/comfy` proxies; CORS still wants cleanup.)
...
- Cenedril's strict perspective/word-count validation **was softened to warnings** (2026-05-25) — it no longer hard-crashes the dream (and block the image stage) on edge cases; it logs and proceeds. Revisit thresholds once telemetry shows where it triggers.
- **Residual image↔video drift:** even with motion-only prompts + gentle camera motion + I2V strength 0.9, longer clips can still wander from the start frame by the end. The real fix (stage-2 latent conditioning) **OOMs the 16 GB card** at the ×2 upscale peak, so it's out; `LTX-Director` (directed motion/continuity) is the candidate, deferred to the interaction milestone.
- **Transient agent parse crashes** (e.g. Narnion "No scenes found", director JSON decode) can kill a whole dream on a flaky LLM parse — wants retry/tolerance. Robustness follow-up.
